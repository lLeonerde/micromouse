...
**Modified by truckershitch for micropython**
...
__repo__ = "https://github.com/truckershitch/Adafruit_CircuitPython_VL53L0X.git"
...
class TimeoutCheck:
    """Very simple sensor timeout checker class"""
...
class VL53L0X:
    ...
    def __init__(self, i2c, address=41, io_timeout_s=0):
        ...
        # with self._i2c:
        # self._i2c.write(ref_spad_map, end=1)
        self._i2c.writeto_mem(self._addr, ref_spad_map[0], ref_spad_map[:1])
        # self._i2c.readinto(ref_spad_map, start=1)
        self._i2c.readfrom_mem_into(self._addr, ref_spad_map[1], ref_spad_map[1:])
        ...
        for i in range(48):
            if i < first_spad_to_enable or spads_enabled == spad_count:
                ...
                spads_enabled += 1
        # with self._i2c:
        # self._i2c.write(ref_spad_map)
        self._i2c.writeto_mem(self._addr, ref_spad_map[0], ref_spad_map)
        for pair in (
            (0xFF, 0x01),
            (0x00, 0x00),
            (0xFF, 0x00),
    ...
    def _reset(self):
        # Reset vl53l0x sensor
        self._i2c.writeto_mem(self._addr, 0x0000, bytes([0x00]), addrsize=16)
        utime.sleep(0.1)
        self._i2c.writeto_mem(self._addr, 0x0000, bytes([0x01]), addrsize=16)

    def _check_for_vl53l0x(self):
        # Checks for presence of VL53L0X sensor
        if (
    ...
    def _read_u8(self, address):
        ...
        # self._i2c.write(self._BUFFER, end=1)
        # self._i2c.readinto(self._BUFFER, end=1)
        self._BUFFER[0] = ord(self._i2c.readfrom_mem(self._addr, address, 1))
    ...
    def _read_u16(self, address):
        ...
        self._BUFFER[0:1] = self._i2c.readfrom_mem(self._addr, address, 2)
    ...
    def _write_u8(self, address, val):
        ...
        self._i2c.writeto_mem(self._addr, self._BUFFER[0], bytes([self._BUFFER[1]]))
    ...
    def _write_u16(self, address, val):
        ...
        self._i2c.writeto_mem(self._addr, self._BUFFER[0], self._BUFFER[1:])
